...
local ZaZaHubReady = false

-- Funktion zum Überwachen des Ladefortschritts
task.spawn(function()
    while not ZaZaHubReady do
        task.wait(0.1)
        -- Fortschritt aktualisieren (z.B. Anzahl geladener Module)
        ProgressText.Text = "Lädt..."
    end
    
    -- Wenn alles geladen ist, GUI ausblenden
    fadeOutElement(Frame).Completed:Connect(function()
        Frame.Visible = false
